...
try {
  ...
  if ($initialTheme.Theme.id -cne 'preset-ssyai-dream' -or
    ...
    throw 'Default Windows theme did not seed the pure ssyai-dream wallpaper contract.'
  }
  $preseededThemes = @(Get-DreamSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or $preseededThemes[0].Id -cne 'preset-ssyai-dream') {
    throw 'ssyai-dream was not preseeded in the Windows saved-theme menu.'
  }
  ...
  # Windows PowerShell 5.1 converts native stderr into a terminating error under
  # $ErrorActionPreference = 'Stop' even with *> $null; this call is expected to fail.
